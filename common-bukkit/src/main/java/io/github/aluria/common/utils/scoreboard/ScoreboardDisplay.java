...
public final class ScoreboardDisplay {
  ...
  static {
    try {
      if (ScoreboardReflectionHelper.nmsOptionalClass("ScoreboardServer$Action").isPresent()) {
        VERSION_TYPE = VersionType.V1_13;
      } else if (ScoreboardReflectionHelper.nmsOptionalClass("IScoreboardCriteria$EnumScoreboardHealthDisplay").isPresent()) {
        VERSION_TYPE = VersionType.V1_8;
      } else {
        VERSION_TYPE = VersionType.V1_7;
      }

      Class<?> craftChatMessageClass = ScoreboardReflectionHelper.obcClass("util.CraftChatMessage");
      Class<?> entityPlayerClass = ScoreboardReflectionHelper.nmsClass("EntityPlayer");
      Class<?> playerConnectionClass = ScoreboardReflectionHelper.nmsClass("PlayerConnection");
      Class<?> craftPlayerClass = ScoreboardReflectionHelper.obcClass("entity.CraftPlayer");

      MESSAGE_FROM_STRING = craftChatMessageClass.getDeclaredMethod("fromString", String.class);
      CHAT_COMPONENT_CLASS = ScoreboardReflectionHelper.nmsClass("IChatBaseComponent");
      ...
      SEND_PACKET = playerConnectionClass.getDeclaredMethod("sendPacket", ScoreboardReflectionHelper.nmsClass("Packet"));

      PACKET_SB_OBJ = ScoreboardReflectionHelper.nmsClass("PacketPlayOutScoreboardObjective").getConstructor();
      PACKET_SB_DISPLAY_OBJ = ScoreboardReflectionHelper.nmsClass("PacketPlayOutScoreboardDisplayObjective").getConstructor();
      PACKET_SB_SCORE = ScoreboardReflectionHelper.nmsClass("PacketPlayOutScoreboardScore").getConstructor();
      PACKET_SB_TEAM = ScoreboardReflectionHelper.nmsClass("PacketPlayOutScoreboardTeam").getConstructor();

      if (VersionType.V1_8.isHigherOrEqual()) {
        ENUM_SB_HEALTH_DISPLAY = ScoreboardReflectionHelper.nmsClass("IScoreboardCriteria$EnumScoreboardHealthDisplay");

        if (VersionType.V1_13.isHigherOrEqual()) {
          ENUM_SB_ACTION = ScoreboardReflectionHelper.nmsClass("ScoreboardServer$Action");
        } else {
          ENUM_SB_ACTION = ScoreboardReflectionHelper.nmsClass("PacketPlayOutScoreboardScore$EnumScoreboardAction");
        }

        ENUM_SB_HEALTH_DISPLAY_INTEGER = ScoreboardReflectionHelper.enumValueOf(ENUM_SB_HEALTH_DISPLAY, "INTEGER");
        ENUM_SB_ACTION_CHANGE = ScoreboardReflectionHelper.enumValueOf(ENUM_SB_ACTION, "CHANGE");
        ENUM_SB_ACTION_REMOVE = ScoreboardReflectionHelper.enumValueOf(ENUM_SB_ACTION, "REMOVE");
      } else {
   ...
   */
  public ScoreboardDisplay(Player player) {
    this.player = Objects.requireNonNull(player, "player");

    id = "sd-" + Double.toString(Math.random()).substring(2, 10);
  ...
  public static class ScoreboardReflectionHelper {
    ...
    private ScoreboardReflectionHelper() {
      throw new UnsupportedOperationException();
    }
